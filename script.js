...
// ==========================================================
// 4.1. BANDEAU D'IMAGES DÉFILANT (MODIFIÉ POUR VITESSE)
// ==========================================================

const imageBandFiles = ['1.jpeg', '2.jpeg', '3.jpeg', '4.jpeg', '5.jpeg'];
const imageBandTop = new THREE.Group();
const imageBandBottom = new THREE.Group();
phase3Group.add(imageBandBottom);

// NOUVELLES DIMENSIONS ET ESPACEMENT (DERNIÈRE AUGMENTATION)
...
const numCopies = 4;
const totalBandWidth = bandSegmentWidth * numCopies;
const bandVerticalOffset = 9.0; 

// Variable de défilement mise à jour par le scroll dans animate()
let bandScrollOffset = 0; 
const SCROLL_COEFFICIENT = 0.02; // DIMINUTION MAJEURE : Était 0.2 -> Défilement TRES lent
    ...
    for (let j = 0; j < numCopies; j++) {
        imageBandFiles.forEach((filename, index) => {
            ...
            const material = new THREE.MeshBasicMaterial({ 
                ...
                // MODIFICATION CLÉ 1 : Définition de la couleur de base sur Blanc ICI
                color: 0xffffff 
            });
            
            // LIGNE SUPPRIMÉE : material.color.set(0xffffff); (Car déjà défini ci-dessus)
            ...
            textureLoader.load(path, (texture) => {
                ...
                console.error('Erreur de chargement du bandeau:', filename, error);
                
                // MODIFICATION CLÉ 2 : En cas d'erreur, on s'assure que c'est Blanc Opaque
...
const bandBottomElements = createRepeatingImageBand(imageBandBottom, -bandVerticalOffset, -1); 

function updateImageBandPhase3(transitionProgress, scrollFactor) {
    const elements = [...bandBottomElements];
    
    // Défilement lié au scroll (scrollFactor * 0.08)
    const scrollMovement = scrollFactor * SCROLL_COEFFICIENT; 
    
    // Le décalage est cyclique
    bandScrollOffset = (scrollMovement) % bandSegmentWidth;
    
    // Gère la boucle de défilement horizontal
    elements.forEach(mesh => {
        ...
        // Opacité : Gérée par la progression (transitionProgress)
        const targetOpacity = Math.min(1, transitionProgress * 3);
        
        if (mesh.material.map) {
            // Assurer que l'opacité est appliquée sur l'image chargée
...
window.updateImageBandPhase3 = updateImageBandPhase3;
...
    // Transition de la Phase 1 (Train) à la Phase 2 (Pinceau)
...
    
    // MODIFIÉ : Réduction TRÈS FORTE du scroll de la Phase 3 (0.3 -> 0.15)
...
        p = Math.min(1, Math.max(0, p));
        ...
        // NOUVEAUTÉ : Progression pour l'apparition du bandeau dans la phase 2
        const bandAppearanceProgress = Math.max(0, Math.min(1, (scrollY - heroHeight) / (heroHeight * 0.5))); 
        
        // Le facteur de défilement pour le bandeau est basé sur la position Y de la fenêtre
        const bandScrollFactor = Math.max(0, scrollY - heroHeight * 0.5); 
        
        // APPEL DU BANDEAU D'IMAGES DÈS LA PHASE 2
        if (window.updateImageBandPhase3) { window.updateImageBandPhase3(bandAppearanceProgress, bandScrollFactor); } 
...
        
        // Le bandeau est maintenant contrôlé par scrollY pur dans la phase 3
        if (window.updateImageBandPhase3) { window.updateImageBandPhase3(1.0, scrollY); } 
        
